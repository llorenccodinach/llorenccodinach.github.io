...
mesas.forEach(element => {
    element.addEventListener("pointerdown", () => {
        ...
        if(element.classList.contains("rest-mesa-selected") || element.style.fill == "rgba(128, 0, 0, 0.21)"){
            ...
            if(windowN != "empresa" || element.style.fill != "rgba(0, 128, 0, 0.21)"){
...
function reservarE() {
    ...
    const mesa = {
        ...
        people: document.getElementById("emp-people-res").value
    }
    ...
    document.getElementById("emp-name-res").value = "";
    document.getElementById("emp-lastname-res").value = "";
    document.getElementById("emp-phone-res").value = "";
    document.getElementById("emp-mail-res").value = "";
    document.getElementById("emp-people-res").value = "";
...
function buscarTaula(){
    ...
    if(reservas[`${inputFecha.value}|${inputHora.value}`] != undefined){
        for (let i = 0; i < reservas[`${inputFecha.value}|${inputHora.value}`].length; i++) {
            ...
            cardMesa.innerHTML = `
            <h5 class="rest-emp-right-card-h5">${element.num.toUpperCase()}</h5>
